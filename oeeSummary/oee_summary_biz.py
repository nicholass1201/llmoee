...
from oeeCb import oee_similarity
from oeeCb import oee_chat_main_schema
from oeeCbMgr import oee_project
from oeeCbMgr.oee_chatbot_models import Project, Intent
from oeeCbMgr import oee_project_schema
from oeeCbMgr import oee_intent_biz
from oeeCbMgr import oee_intent_schema
 
from oeeCb.oeeLlmCb101 import oee_chat_sub_llm_predict_101
from oeeCb.oeeLlmCb102 import oee_chat_sub_llm_predict_102
from oeeCb.oeeLlmCb201 import oee_chat_sub_llm_predict_201
# from oeeCb.oeetLlmCb202 import oee_chat_sub_llm_predict_202
...
def chatMain_predict(company_id:str,project_id:int, query:str, before_intent_id:int):
    ...
    oee_intent_schema.IntentSrchParam.intent_id = "120 " ; 
    ...
    total_cnt, _intent_list = oee_intent_biz.get_intent_list( oee_intent_schema.IntentSrchParam)        
    print("total_cnt: ", total_cnt)
    print("_intent_list: ", _intent_list)    

    strStopKeyword="" ; 
    strSourceCd = "" ; 

    for row in _intent_list:
        print("========_intent_row.project_id: ", row.project_id)        
        print("========_intent_row.company_id: ", row.company_id)
        print("========_intent_row.intent_id: ", row.intent_id)
    ...
    # oee_similarity.makeCosSimilarArr(arrChatQuery, query, strFindKeyword , strStopKeyword, containNounYn)   # oee_similarity.
    ...
    if (strSourceCd==""):
        ...
        if (project_id==36):
            ...
            response_result = oee_chat_sub_llm_predict_102.biz_rag_predict(company_id, project_id, query=query, before_intent_id=0)                
        elif (project_id==37):            
            print("============llm201 project_id 37일때: ", project_id)
            response_result = oee_chat_sub_llm_predict_201.biz_rag_predict(company_id, project_id, query=query, before_intent_id=0)                
        elif (project_id==38):            
            ...
            print("============llm101 project_id 36아닐때: ", project_id)            
...
def chatMain2_predict(company_id:str,project_id:int, query:str, before_intent_id:int):
    print("===================== chatMain_predict start ")        
    response_result=""
    arrChatQuery=[] 
    ...
    if (strSourceCd==""):
        ...
        print("================make_chat_main biz_rag_predict query: " , query)        
        #response_result = "하드코response_result딩 답변입니다."
        response_result = oee_chat_sub_llm_predict_101.biz_rag_predict(project_id=1, query=query, before_intent_id=0)    
        print("chatMain_app chat_rag_predict response_result: " , response_result)      
